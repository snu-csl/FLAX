...
#define IO_WORKER_COUNT 5

static struct ioat_dma_thread dma_thread[IO_WORKER_COUNT];
...
int ioat_dma_submit(int id, dma_addr_t src_addr, dma_addr_t dst_addr, unsigned int size)
{
	struct ioat_dma_thread *thread = &(dma_thread[id]);
	struct ioat_dma_info *info;
	struct dma_chan *chan;
...
static int ioat_dma_add_channel(int id, struct ioat_dma_info *info, struct dma_chan *chan)
{
	...
	if (dma_has_cap(DMA_MEMCPY, dma_dev->cap_mask)) {
		...
		dma_thread[id].info = info;
		dma_thread[id].chan = dtc->chan;
		dma_thread[id].type = DMA_MEMCPY;
	}

	pr_info("Added %s to thread %d\n", dma_chan_name(chan), id);
...
static void request_channels(int id, struct ioat_dma_info *info, enum dma_transaction_type type)
{
	dma_cap_mask_t mask;
	...
	for (;;) {
		...
		if (chan) {
			if (ioat_dma_add_channel(id, info, chan)) {
...
static void add_threaded_dma(int id, struct ioat_dma_info *info)
{
	struct ioat_dma_params *params = &info->params;
	...
	request_channels(id, info, DMA_MEMCPY);
}
...
int ioat_dma_chan_set(const char *val)
{
	...
	for (int i = 0; i < IO_WORKER_COUNT; i++) {
		...
		sprintf(test_channel, "dma%dchan0", i);
