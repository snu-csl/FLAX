...
static void ssd_init_nand_page(struct nand_page *pg, struct ssdparams *spp)
{
	...
	pg->sec = kmalloc_node(sizeof(nand_sec_status_t) * pg->nsecs, GFP_KERNEL, 1);
...
static void ssd_init_nand_blk(struct nand_block *blk, struct ssdparams *spp)
{
	...
	blk->pg = kmalloc_node(sizeof(struct nand_page) * blk->npgs, GFP_KERNEL, 1);
...
static void ssd_init_nand_plane(struct nand_plane *pl, struct ssdparams *spp)
{
	...
	pl->blk = kmalloc_node(sizeof(struct nand_block) * pl->nblks, GFP_KERNEL, 1);
...
static void ssd_init_nand_lun(struct nand_lun *lun, struct ssdparams *spp)
{
	...
	lun->pl = kmalloc_node(sizeof(struct nand_plane) * lun->npls, GFP_KERNEL, 1);
	for (i = 0; i < lun->npls; i++) {
		ssd_init_nand_plane(&lun->pl[i], spp);
	}
	lun->next_lun_avail_read_time = 0;
	lun->next_lun_avail_write_time = 0;
	lun->busy = false;
}
...
void ssd_init_ch(struct ssd_channel *ch, struct ssdparams *spp)
{
	...
	ch->lun = kmalloc_node(sizeof(struct nand_lun) * ch->nluns, GFP_KERNEL, 1);
	for (i = 0; i < ch->nluns; i++) {
		ssd_init_nand_lun(&ch->lun[i], spp);
	}

	ch->perf_model = kmalloc_node(sizeof(struct channel_model), GFP_KERNEL, 1);
...
{
	pcie->perf_model = kmalloc_node(sizeof(struct channel_model), GFP_KERNEL, 1);
	chmodel_init(pcie->perf_model, spp->pcie_bandwidth);
}

static void __maybe_unused ssd_remove_pcie(struct ssd_pcie *pcie)
{
...
void ssd_init(struct ssd *ssd, struct ssdparams *spp, uint32_t cpu_nr_dispatcher)
{
	...
	ssd->ch = kmalloc_node(sizeof(struct ssd_channel) * spp->nchs, GFP_KERNEL, 1); // 40 * 8 = 320
	...
	ssd->pcie = kmalloc_node(sizeof(struct ssd_pcie), GFP_KERNEL, 1);
	ssd_init_pcie(ssd->pcie, spp);

	ssd->write_buffer = kmalloc_node(sizeof(struct buffer), GFP_KERNEL, 1);
...
*/
uint64_t ssd_advance_write_buffer(struct ssd *ssd, uint64_t request_time, uint64_t length, bool interleave_pci_dma)
{
...
uint64_t ssd_advance_nand(struct ssd *ssd, struct nand_cmd *ncmd)
{
	...
	NVMEV_DEBUG("SSD: %p, Enter stime: %lld, ch %lu lun %lu blk %lu page %lu command %d ppa 0x%llx\n", ssd, ncmd->stime,
				ppa->g.ch, ppa->g.lun, ppa->g.blk, ppa->g.pg, c, ppa->ppa);
	...
	case NAND_READ:
		...
		nand_stime = (lun->next_lun_avail_read_time < cmd_stime) ? cmd_stime : lun->next_lun_avail_read_time;
		if (ncmd->nand_stime < nand_stime) {
			ncmd->nand_stime = nand_stime;
		}
		...
		lun->next_lun_avail_read_time = chnl_etime;
		break;

	case NAND_WRITE:
		/* write: transfer data through channel first */
		chnl_stime = (lun->next_lun_avail_write_time < cmd_stime) ? cmd_stime : lun->next_lun_avail_write_time;
		...
		lun->next_lun_avail_write_time = nand_etime;
		completed_time = nand_etime;
		break;

	case NAND_ERASE:
		/* erase: only need to advance NAND status */
		nand_stime = (lun->next_lun_avail_write_time < cmd_stime) ? cmd_stime : lun->next_lun_avail_write_time;
	...
	case NAND_NOP:
		...
		nand_stime = (lun->next_lun_avail_write_time < cmd_stime) ? cmd_stime : lun->next_lun_avail_write_time;
		lun->next_lun_avail_write_time = nand_stime;
...
uint64_t ssd_next_idle_time(struct ssd *ssd)
{
	...
	for (i = 0; i < spp->nchs; i++) {
		...
		for (j = 0; j < spp->luns_per_ch; j++) {
			...
			latest = max(latest, lun->next_lun_avail_write_time);
		}
	}
