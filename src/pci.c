...
static void __maybe_unused __init_apicid_to_cpuid(void)
{
	int i;
...
static void __process_msi_irq(int msi_index)
{
	...
	for_each_msi_entry_safe(msi_desc, tmp, (&vdev->dev)) {
...
static int nvmev_pci_read(struct pci_bus *bus, unsigned int devfn, int where, int size, u32 *val)
{
	...
	NVMEV_DEBUG("[R] 0x%x, size: %d, val: 0x%x\n", where, size, *val);
	...
	if (where < OFFS_PCI_PM_CAP) {
		...
		if (target == PCI_COMMAND) {
			...
			if ((val ^ _val) & PCI_COMMAND_INTX_DISABLE) {
				vdev->intx_disabled = !!(_val & PCI_COMMAND_INTX_DISABLE);
		...
		if (target == PCI_MSIX_FLAGS) {
			...
			if ((vdev->pdev) && ((val ^ _val) & PCI_MSIX_FLAGS_ENABLE)) {
				vdev->pdev->msix_enabled = !!(_val & PCI_MSIX_FLAGS_ENABLE);
			}
		} else {
			mask = 0x0;
		}
...
static struct pci_bus *__create_pci_bus(void)
{
	...
	nvmev_pci_sysdata.node = cpu_to_node(vdev->config.cpu_nr_dispatcher);
	...
	list_for_each_entry(dev, &bus->devices, bus_list) {
		...
		vdev->old_dbs = kzalloc_node(PAGE_SIZE, GFP_KERNEL, 1);
		BUG_ON(!vdev->old_dbs && "allocating old DBs memory");
		memcpy(vdev->old_dbs, vdev->dbs, sizeof(*vdev->old_dbs));

		vdev->old_bar = kzalloc_node(PAGE_SIZE, GFP_KERNEL, 1);
...
struct nvmev_dev *VDEV_INIT(void)
{
	...
	vdev = kzalloc_node(sizeof(*vdev), GFP_KERNEL, 1);

	vdev->virtDev = kzalloc_node(PAGE_SIZE, GFP_KERNEL, 1);
...
void VDEV_FINALIZE(struct nvmev_dev *vdev)
{
	...
	if (vdev->admin_q) {
		if (vdev->admin_q->nvme_cq)
			kfree(vdev->admin_q->nvme_cq);

		if (vdev->admin_q->nvme_sq)
