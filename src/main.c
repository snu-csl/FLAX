...
static int __proc_file_read(struct seq_file *m, void *data)
{
	...
	if (strcmp(filename, "read_times") == 0) {
		...
		int tasks = get_running_task_count();
		int io_reqs = get_used_io_req_count();
		int slm = buddy_is_allocated();
		int has_leak = (tasks > 0 || io_reqs > 0 || slm);
		seq_printf(m, "%d\n", has_leak);
		seq_printf(m, "tasks: %d/%d\n", tasks, MAX_TASK_COUNT);
		seq_printf(m, "io_reqs: %d/%d\n", io_reqs, MAX_INTERNAL_IO_COUNT);
		seq_printf(m, "slm_allocated: %d\n", slm);
		for (int i = 1; i <= vdev->nr_sq; i++) {
			struct nvmev_submission_queue *sq = vdev->sqes[i];
			if (!sq)
				continue;
			seq_printf(m, "sq[%d] deferred: %u\n", i, sq->deferred_count);
		}
#ifdef CONFIG_NVMEV_PROFILE_IO
	} else if (strcmp(filename, "io_stat") == 0) {
		seq_printf(m, "host read_bytes: %lld\n", atomic64_read(&vdev->io_stat_host_read));
		seq_printf(m, "host write_bytes: %lld\n", atomic64_read(&vdev->io_stat_host_write));
		seq_printf(m, "csd read_bytes: %lld\n", atomic64_read(&vdev->io_stat_csd_read));
		seq_printf(m, "csd write_bytes: %lld\n", atomic64_read(&vdev->io_stat_csd_write));
#endif
	} else if (strcmp(filename, "deferred") == 0) {
		for (int i = 1; i <= vdev->nr_sq; i++) {
			struct nvmev_submission_queue *sq = vdev->sqes[i];
			struct nvmev_deferred_io *dio;
			if (!sq || sq->deferred_count == 0)
				continue;
			seq_printf(m, "sq[%d] (%u cmds):", i, sq->deferred_count);
			list_for_each_entry(dio, &sq->deferred_list, list) {
				seq_printf(m, " 0x%02x", dio->cmd.common.opcode);
			}
			seq_printf(m, "\n");
		}
	}
...
#ifdef CONFIG_NVMEV_PROFILE_IO
	} else if (!strcmp(filename, "io_stat")) {
		atomic64_set(&vdev->io_stat_host_read, 0);
		atomic64_set(&vdev->io_stat_host_write, 0);
		atomic64_set(&vdev->io_stat_csd_read, 0);
		atomic64_set(&vdev->io_stat_csd_write, 0);
		NVMEV_INFO("Reset I/O stat\n");
#endif
...
void NVMEV_STORAGE_INIT(struct nvmev_dev *vdev)
{
	...
	vdev->proc_deferred = proc_create("deferred", 0444, vdev->proc_root, &proc_file_fops);
	vdev->proc_ebpf = proc_create("ebpf", 0664, vdev->proc_root, &proc_file_fops);
#ifdef CONFIG_NVMEV_PROFILE_IO
	vdev->proc_io_stat = proc_create("io_stat", 0644, vdev->proc_root, &proc_file_fops);
#endif
}
...
void NVMEV_STORAGE_FINAL(struct nvmev_dev *vdev)
{
	...
	remove_proc_entry("deferred", vdev->proc_root);
	remove_proc_entry("ebpf", vdev->proc_root);
#ifdef CONFIG_NVMEV_PROFILE_IO
	remove_proc_entry("io_stat", vdev->proc_root);
#endif
...
static bool __load_configs(struct nvmev_config *config)
{
	...
	while ((cpu = strsep(&csd_cpus, ",")) != NULL) {
		cpu_nr = (unsigned int)simple_strtol(cpu, NULL, 10);
		if (count == 0) {
			config->cpu_nr_csd_dispatcher = cpu_nr;
		} else if (count == 1) {
			config->cpu_nr_csd_helper = cpu_nr;
		} else {
