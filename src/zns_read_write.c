...
bool __zns_write(struct zns_ftl *zns_ftl, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	for (lpn = slpn; lpn <= elpn; lpn += pgs) {
		...
		if ((pg_off + pgs) == spp->pgs_per_oneshotpg) {
			...
			enqueue_writeback_io_req(req->sq_id, nsecs_completed, zns_ftl->ssd->write_buffer,
...
out:
	ret->status = status;
	if ((cmd->control & NVME_RW_FUA) || (spp->write_early_completion == 0)) /*Wait all flash operations*/
...
bool __zns_write_zrwa(struct zns_ftl *zns_ftl, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	if (elba >= zrwa_impl_start) {
		...
		NVMEV_DEBUG("%s implicitly flush zid %d wp before 0x%llx after 0x%llx buffer %d", __FUNCTION__, zid, prev_wp,
					zone_descs[zid].wp + nr_lbas_flush, zns_ftl->zwra_buffer[zid].remaining);
		...
		NVMEV_DEBUG("%s end of zone zid %d wp before 0x%llx after 0x%llx buffer %d", __FUNCTION__, zid, prev_wp,
					zone_descs[zid].wp + nr_lbas_flush, zns_ftl->zwra_buffer[zid].remaining);
	}
	...
	while (remaining > 0) {
		...
		if ((pg_off + pgs) == spp->pgs_per_oneshotpg) {
			...
			enqueue_writeback_io_req(req->sq_id, nsecs_completed, &zns_ftl->zwra_buffer[zid],
									 spp->pgs_per_oneshotpg * spp->pgsz);
		}
...
bool zns_read(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	NVMEV_ZNS_DEBUG("%s slba 0x%llx nr_lba 0x%lx zone_id %d state %d wp 0x%llx last lba 0x%llx\n", __FUNCTION__, slba,
					nr_lba, zid, zone_descs[zid].state, zone_descs[zid].wp, (slba + nr_lba - 1));
