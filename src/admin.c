...
static void __nvmev_admin_create_cq(int eid)
{
	...
	cq = kzalloc_node(sizeof(struct nvmev_completion_queue), GFP_KERNEL, 1);
	...
	cq->cq = kzalloc_node(sizeof(struct nvme_completion *) * num_pages, GFP_KERNEL, 1);
...
static void __nvmev_admin_create_sq(int eid)
{
	...
	sq = kzalloc_node(sizeof(struct nvmev_submission_queue), GFP_KERNEL, 1);
	...
	sq->sq = kzalloc_node(sizeof(struct nvme_command *) * num_pages, GFP_KERNEL, 1);
...
static void __nvmev_admin_identify_zns_namespace(int eid)
{
	...
	BUG_ON(vdev->ns[nsid].csi != NVME_CSI_ZNS);
...
static void __nvmev_admin_set_features(int eid)
{
	...
	case NVME_FEAT_NUM_QUEUES: {
		...
		result0 = ((vdev->nr_cq - 1) << 16 | (vdev->nr_sq - 1));
		break;
	}
...
static void __nvmev_proc_admin_req(int entry_id)
{
	...
	NVMEV_DEBUG("%s: %d 0x%x 0x%x\n", __func__, entry_id, sqe->common.opcode, sqe->common.command_id);
