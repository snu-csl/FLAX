											  ...
											  unsigned long long nsecs_start)
{
	...
	if (opcode == nvme_cmd_write || opcode == nvme_cmd_kv_store || opcode == nvme_cmd_kv_batch) {
...
size_t allocate_mem_offset(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd)
{
	if (cmd.common.opcode == nvme_cmd_kv_store) {
		...
		if (offset == -1) {
			...
			NVMEV_DEBUG("allocate memory offset %llu for %u %u\n", offset, cmd_key_length(cmd), cmd_value_length(cmd));
			return offset;
		}
	} else {
...
size_t allocate_mem_offset_by_length(struct kv_ftl *kv_ftl, int val_len)
{
	...
	if (offset == -1) {
		...
		NVMEV_DEBUG("allocate memory offset %llu for %u\n", offset, val_len);
		return offset;
	}
}
...
unsigned int new_mapping_entry(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, size_t val_offset)
{
	...
	NVMEV_DEBUG("New mapping entry key %s offset %llu length %u slot %u\n", cmd.kv_store.key, val_offset,
				cmd_value_length(cmd), slot);

	return 0;
}

unsigned int new_mapping_entry_by_key(struct kv_ftl *kv_ftl, unsigned char *key, int key_len, int val_len,
									  size_t val_offset)
{
	unsigned int slot = -1;
	unsigned int prev_slot;
...
unsigned int update_mapping_entry(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd)
{
	...
	while (kv_ftl->kv_mapping_table[slot].mem_offset != -1) {
		NVMEV_DEBUG("Comparing %s | %.*s\n", cmd.kv_store.key, cmd_key_length(cmd), kv_ftl->kv_mapping_table[slot].key);
		count++;
		...
		if (memcmp(cmd.kv_store.key, kv_ftl->kv_mapping_table[slot].key, cmd_key_length(cmd)) == 0) {
			NVMEV_DEBUG("1 Found\n");
			found = true;
			break;
		}
	...
	if (found) {
		NVMEV_DEBUG("Updating mapping length %llu to %u for key %s\n", kv_ftl->kv_mapping_table[slot].length,
					cmd_value_length(cmd), cmd.kv_store.key);
		kv_ftl->kv_mapping_table[slot].length = cmd_value_length(cmd);
	}
...
struct mapping_entry get_mapping_entry(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd)
{
	...
	if (!found)
		...
		NVMEV_DEBUG("Returning mapping %llu length %llu for key %s\n", mapping.mem_offset, mapping.length,
					cmd.kv_store.key);

	return mapping;
}

struct mapping_entry get_mapping_entry_by_key(struct kv_ftl *kv_ftl, unsigned char *key, int key_len)
{
	struct mapping_entry mapping;
	// char *key = NULL;
	unsigned int slot = 0;
	...
	while (kv_ftl->kv_mapping_table[slot].mem_offset != -1) {
		NVMEV_DEBUG("Comparing %s | %.*s\n", key, key_len, kv_ftl->kv_mapping_table[slot].key);
		count++;
 ...
 */
static unsigned int __do_perform_kv_io(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, unsigned int *status)
{
	size_t offset;
	...
	if (cmd.common.opcode == nvme_cmd_kv_store) {
		...
		if (entry.mem_offset == -1) { // kv pair doesn't exist
			...
			*status = KV_ERR_KEY_NOT_EXIST;
			...
			NVMEV_DEBUG("kv_delete %s exist - length %ld, offset %llu\n", cmd.kv_store.key, length, offset);
...
static unsigned int __do_perform_kv_batched_io(struct kv_ftl *kv_ftl, int opcode, char *key, int key_len, char *value,
											   int val_len)
{
	size_t offset;
	size_t new_offset = 0;
...
static unsigned int __do_perform_kv_batch(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, unsigned int *status)
{
	size_t offset;
	size_t length, remaining;
	int prp_offs = 0;
	...
	for (i = 0; i < sub_cmd_cnt; i++) {
		...
		memcpy(value, payload->sub_payload + payload_offset + ((key_len - 1) / ALIGN_LEN + 1) * ALIGN_LEN, val_len);
		payload_offset += sub_len;
		NVMEV_DEBUG("sub-payload %d %d %d %d %s %s", payload->batch_head.attr[i].opcode, key_len, val_len, sub_len, key,
					value);
...
static unsigned int kv_iter_read(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, unsigned int *status)
{
	int iter = cmd.kv_iter_req.iter_handle;
	...
	while (pos < kv_ftl->hash_slots) {
		if (kv_ftl->kv_mapping_table[pos].mem_offset != -1) {
			...
			if ((key & handle->bitmask) == (handle->prefix & handle->bitmask)) {
				NVMEV_DEBUG("found %s at %d", kv_ftl->kv_mapping_table[pos].key, pos);
				...
				buf_offset += 4;
				memcpy(handle->buf + buf_offset, kv_ftl->kv_mapping_table[pos].key, keylen);
...
static unsigned int __do_perform_kv_iter_io(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, unsigned int *status)
{
...
bool kv_proc_nvme_io_cmd(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	case nvme_cmd_write:
	case nvme_cmd_read:
		ret->nsecs_target = __schedule_io_units(cmd->common.opcode, cmd->rw.slba,
	...
	case nvme_cmd_kv_batch:
		ret->nsecs_target = __schedule_io_units(cmd->common.opcode, 0,
												cmd_value_length(*((struct nvme_kv_command *)cmd)), __get_wallclock());
...
void kv_init_namespace(struct nvmev_ns *ns, uint32_t id, uint64_t size, void *mapped_addr, uint32_t cpu_nr_dispatcher)
{
	...
	kv_ftl->kv_mapping_table =
		memremap(vdev->config.storage_start + vdev->config.storage_size, KV_MAPPING_TABLE_SIZE << 20, MEMREMAP_WB);
