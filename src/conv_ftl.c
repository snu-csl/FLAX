...
static bool __maybe_unused should_gc(struct conv_ftl *conv_ftl)
{
...
static void __maybe_unused clean_one_block(struct conv_ftl *conv_ftl, struct ppa *ppa)
{
	struct ssdparams *spp = &conv_ftl->ssd->sp;
		...
		for (ch = 0; ch < spp->nchs; ch++) {
			for (lun = 0; lun < spp->luns_per_ch; lun++) {
				...
				if (flashpg == (spp->flashpgs_per_blk - 1)) {
					...
					lunp->gc_endtime = lunp->next_lun_avail_write_time;
				}
			}
		}
	}
...
enum nvmev_io_status conv_read(struct nvmev_ns *ns, struct nvmev_request *req,
...
enum nvmev_io_status conv_write(struct nvmev_ns *ns, struct nvmev_request *req,
				struct nvmev_result *ret)
{
	struct conv_ftl *conv_ftls = (struct conv_ftl *)ns->ftls;
	struct conv_ftl *conv_ftl = &conv_ftls[0];
	...
	if (allocated_buf_size < LBA_TO_BYTE(nr_lba))
		return NVMEV_IO_STATUS_DEFERRED;
...
enum nvmev_io_status conv_namespace_copy(struct nvmev_ns *ns, struct nvmev_request *req,
					 struct nvmev_result *ret)
{
	struct nvme_command_csd *cmd = (struct nvme_command_csd *)req->cmd;
	...
	while (remain > 0) {
		...
		if (dummy_cmd.rw.opcode == nvme_cmd_write) {
			...
			status = conv_write(ns, &dummy_req, &dummy_ret);
			if (status != NVMEV_IO_STATUS_SUCCESS) {
				ret->nsecs_target = -1;
				return status;
			}
		} else if (dummy_cmd.rw.opcode == nvme_cmd_read) {
			status = conv_read(ns, &dummy_req, &dummy_ret);
			if (status != NVMEV_IO_STATUS_SUCCESS) {
    	   	ret->nsecs_target = -1;
				return status;
			}
...
enum nvmev_io_status conv_proc_nvme_io_cmd(struct nvmev_ns *ns, struct nvmev_request *req,
					   struct nvmev_result *ret)
{
	struct nvme_command *cmd = req->cmd;
	...
	case nvme_cmd_write:
		status = conv_write(ns, req, ret);
		break;
	case nvme_cmd_read:
		status = conv_read(ns, req, ret);
		break;
	case nvme_cmd_flush:
		conv_flush(ns, req, ret);
		status = NVMEV_IO_STATUS_SUCCESS;
		break;
	...
	case nvme_cmd_namespace_copy:
		status = conv_namespace_copy(ns, req, ret);
		break;
	default:
		break;
	}
